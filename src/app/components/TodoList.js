...
export default function EnhancedTable() {
  ...
  const [orderBy, setOrderBy] = React.useState('dueBy');
  const [selected, setSelected] = React.useState([]);
  const [rows, setRows] = React.useState([]);
  const [page, setPage] = React.useState(0);
  const [rowsPerPage, setRowsPerPage] = React.useState(5);
  const todoList = useSelector(store => store.todo.tasks);

  useEffect(() => {
    console.log(todoList)
    let data = todoList.map((task) => {
      const { summary, priority, createdOn, dueBy } = task;
      return createData(summary, priority, createdOn, dueBy)
    })
    setRows(data);
  }, [todoList])
  ...
  function Row(row, index) {
    const isItemSelected = isSelected(row.summary);
